...
class RecSpec:
    """
    Compares the user's system information to the recommended
    specifications to run a selected application or game.
    ...
    Methods:
        update_recspec(select_games, result_text): Fetches recommended specifications from a database and
    ...
    def update_recspec(self, select_games, result_text):
        """
        Retrieves the recommended specifications for the selected application or game from the database,
        ...
        with pyodbc.connect(
            ...
            with conn.cursor() as cursor:
                # Execute queries to fetch the recspec values for the selected application
                cursor.execute("SELECT recCPU FROM specsRec WHERE recspecsID = ?", app_id)
                rec_cpu_cores = cursor.fetchone()[0]
                cursor.execute("SELECT recSpeed FROM specsRec WHERE recspecsID = ?", app_id)
                cpu_speed = cursor.fetchone()[0]
                cursor.execute("SELECT recMem FROM specsRec WHERE recspecsID = ?", app_id)
                rec_ram = cursor.fetchone()[0]
                cursor.execute("SELECT recStorage FROM specsRec WHERE recspecsID = ?", app_id)
                rec_storage = cursor.fetchone()[0]
                cursor.execute("SELECT recVRAM FROM specsRec WHERE recspecsID = ?", app_id)
                rec_vram = cursor.fetchone()[0]
                ...
                # Compare CPU cores
                cpu_string = f"Recommended CPU cores: {rec_cpu_cores}\n"
                cpu_string += f"Current CPU cores: {current_cpu_cores}\n"
                cpu_string += f"Recommended CPU speed: {cpu_speed:.2f} GHz\n"
                cpu_string += f"Current CPU speed: {current_cpu_speed} GHz\n"
                if current_cpu_cores < rec_cpu_cores:
                    cpu_1_check = False
                    cpu_2_check = False
                    cpu_result_string = f"Your CPU is below the recommended specs. " \
                                        f"Here are some products to consider: " \
                                        f"https://www.newegg.com/p/pl?d={rec_vram:.0f}+core+{cpu_speed:.2f}ghz+cpu\n\n"
                elif current_cpu_cores == rec_cpu_cores:
                    # Compare CPU speed
                    if current_cpu_speed < cpu_speed:
                        cpu_1_check = True
                        cpu_2_check = False
                        cpu_result_string = f"Your CPU speed is below the recommended specs. " \
                                            f"Here are some products to consider: " \
                                            f"https://www.newegg.com/p/pl?d={rec_vram:.0f}+core+{cpu_speed:.2f}" \
                                            f"ghz+cpu\n\n"
                    else:
                        cpu_1_check = True
                        cpu_2_check = True
                        cpu_result_string = "Your CPU meets the recommended specs.\n\n"
                else:
                    cpu_1_check = True
                    cpu_2_check = True
                    cpu_result_string = "Your CPU meets the recommended specs.\n\n"
                ...
                # Compare RAM
                ram_string = f"Recommended RAM: {rec_ram:.2f} GB\n"
                ram_string += f"Current RAM: {current_ram:.2f} GB\n"
                if current_ram < rec_ram:
                    ram_check = False
                    ram_result_string = f"Your RAM is below the recommended specs. " \
                                        f"Here are some products to consider: " \
                                        f"https://www.newegg.com/p/pl?d={rec_ram:.0f}gb+ram+card\n\n"
                else:
                    ram_check = True
                    ram_result_string = "Your RAM meets the recommended specs.\n\n"

                # Compare VRAM
                vram_string = f"Recommended VRAM: {rec_vram:.2f} GB \n"
                vram_string += f"Current VRAM: {vram} GB \n"
                if vram < rec_vram:
                    vram_check = False
                    vram_result_string = f"Your VRAM is below the recommended specs. " \
                                         f"Here are some products to consider: " \
                                         f"https://www.newegg.com/p/pl?d={rec_vram:.0f}gb+graphics+card\n\n"
                else:
                    vram_check = True
                    vram_result_string = "Your VRAM meets the recommended specs.\n\n"

                # Compare free storage
                size_string = f"Recommended free storage required: {rec_storage:.2f}\n"
                size_string += f"Current free storage: {storage_size:.2f} GB\n"
                if storage_size < rec_storage:
                    size_check = False
                    size_result_string = f"You do not have enough storage. " \
                                         f"Here are some products to consider: " \
                                         f"https://www.newegg.com/p/pl?d={rec_storage:.0f}gb+ssd\n\n"
                else:
                    size_check = True
                    size_result_string = "You have enough free storage."

                # Enable result text and delete current content to clear2
